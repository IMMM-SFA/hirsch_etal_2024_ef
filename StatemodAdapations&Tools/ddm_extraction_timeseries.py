...
estes_park_indoor_demand_df = df.loc[df['id'] == '0400518_I']
estes_park_indoor_demand_df['yr'] = estes_park_indoor_demand_df['yr'].astype(int)
print(estes_park_indoor_demand_df.dtypes)
estes_park_indoor_demand_df_selection = estes_park_indoor_demand_df.iloc[46:63, 2:14]
estes_park_indoor_demand_df_selection_update = pd.DataFrame(estes_park_indoor_demand_df_selection.values.ravel(),columns = ['column'])
estes_park_indoor_demand_df_selection_update['date'] = pd.date_range(start='1/1/1996', periods=len(estes_park_indoor_demand_df_selection_update), freq='M')
estes_park_indoor_demand_df_selection_update.index = estes_park_indoor_demand_df_selection_update['date']
estes_park_indoor_demand_df_selection_update['month'] = estes_park_indoor_demand_df_selection_update.index.month
estes_park_indoor_demand_df_selection_update['year'] = estes_park_indoor_demand_df_selection_update.index.year
estes_park_indoor_demand_df_selection_update['norm'] = np.nan



yearly_sum = estes_park_indoor_demand_df_selection_update.groupby('year').sum()['column']


for i in range(0,204):
    year_check = yearly_sum.index == estes_park_indoor_demand_df_selection_update['year'][i]
    annual_sum = yearly_sum[year_check]
    estes_park_indoor_demand_df_selection_update['norm'][i] = estes_park_indoor_demand_df_selection_update['column'][i]/annual_sum

monthly_mean = estes_park_indoor_demand_df_selection_update.groupby('month').mean()['norm']
monthly_std = estes_park_indoor_demand_df_selection_update.groupby('month').std()['norm']
monthly_std

estes_park_indoor_demand_df_selection_update['deseas'] = estes_park_indoor_demand_df_selection_update['norm'].copy()
for i in range(1, 13):
    mu = monthly_mean[i]
    sigma = monthly_std[i]
    estes_park_indoor_demand_df_selection_update['deseas'].loc[estes_park_indoor_demand_df_selection_update['month'] == i] = (estes_park_indoor_demand_df_selection_update['deseas'].loc[estes_park_indoor_demand_df_selection_update['month'] == i] - mu) / sigma
    
from statsmodels.graphics.tsaplots import plot_acf
# plt.figure()
# fig = plot_acf(estes_park_indoor_demand_df_selection_update['deseas'])

estes_park_indoor_demand_df_selection_update = estes_park_indoor_demand_df_selection_update.fillna(0)

estes_park_indoor_demand_df_selection_update['deseas_l1'] = np.nan
estes_park_indoor_demand_df_selection_update['deseas_l12'] = np.nan
print(estes_park_indoor_demand_df_selection_update.head())
print()

estes_park_indoor_demand_df_selection_update['deseas_l1'].iloc[1:] = estes_park_indoor_demand_df_selection_update['deseas'].values[:-1]
estes_park_indoor_demand_df_selection_update['deseas_l12'].iloc[12:] = estes_park_indoor_demand_df_selection_update['deseas'].values[:-12]

estes_park_indoor_demand_df_selection_update.head

lm_log_ar = sm.ols('deseas ~ deseas_l1 + deseas_l12', data=estes_park_indoor_demand_df_selection_update)
lm_log_ar_fit = lm_log_ar.fit()
print(lm_log_ar_fit.summary())

estes_park_indoor_demand_df_selection_update['ar_resid'] = lm_log_ar_fit.resid
print(estes_park_indoor_demand_df_selection_update)

# plt.figure()
# plt.plot(estes_park_indoor_demand_df_selection_update['ar_resid'])

# plt.figure()
# plt.hist(estes_park_indoor_demand_df_selection_update['ar_resid'])

# plt.figure()
# fig = plot_acf(estes_park_indoor_demand_df_selection_update['ar_resid'].iloc[12:])

estes_park_indoor_demands = pd.DataFrame()


from scipy.stats import skewnorm, gamma, norm
estes_park_indoor_demands['synthetic2_noise'] = norm.rvs(estes_park_indoor_demand_df_selection_update['ar_resid'].mean(), estes_park_indoor_demand_df_selection_update['ar_resid'].std(), size=synthetic_series_length)

# plt.figure()
# plt.plot(estes_park_indoor_demand_df_selection_update['ar_resid'])
# plt.plot(estes_park_indoor_demands['synthetic2_noise'])

def predict_ar(lag1, lag12):
    return lm_log_ar_fit.params[0] + lm_log_ar_fit.params[1] * lag1 + lm_log_ar_fit.params[2] * lag12

max_lag = 12
nrow = estes_park_indoor_demands.shape[0]

synth_ar = list(estes_park_indoor_demand_df_selection_update['deseas'].iloc[-max_lag:])
synth_ar
...
for i in range(nrow):
    ...
    lag1 = synth_ar[i + max_lag - 1]
    prediction = predict_ar(lag1, lag12)
    noise = estes_park_indoor_demands['synthetic2_noise'].iloc[i]
    synth_ar.append(prediction + noise)
    
estes_park_indoor_demands['synthetic2_deseas'] = synth_ar[-nrow:]
estes_park_indoor_demands

estes_park_indoor_demands['date'] = pd.date_range(start='1/1/1950', periods=len(estes_park_indoor_demands), freq='M')
estes_park_indoor_demands.index = estes_park_indoor_demands['date']
estes_park_indoor_demands['month'] = estes_park_indoor_demands.index.month
estes_park_indoor_demands['year'] = estes_park_indoor_demands.index.year


estes_park_indoor_demands['synthetic_demand'] = estes_park_indoor_demands['synthetic2_deseas'].copy()
for i in range(1, 13):
    mu = monthly_mean[i]
    sigma = monthly_std[i]
    estes_park_indoor_demands['synthetic_demand'].loc[estes_park_indoor_demands['month'] == i] = estes_park_indoor_demands['synthetic_demand'].loc[estes_park_indoor_demands['month'] == i] * sigma + mu
    
estes_park_indoor_demands['6000'] = estes_park_indoor_demands['synthetic_demand']*6000

# plt.figure()
# plt.plot(estes_park_indoor_demands['synthetic_demand'])


# plt.figure()
# plt.title('Estes Park Indoor Demands')
# plt.plot(estes_park_indoor_demands['6000'])

# plt.figure()
# plt.title('Estes Park Indoor Historical')
# plt.plot(estes_park_indoor_demand_df_selection_update['column'])


#ESTES PARK OUTDOOR

estes_park_outdoor_demand_df = df.loc[df['id'] == '0400518_O']
estes_park_outdoor_demand_df['yr'] = estes_park_outdoor_demand_df['yr'].astype(int)
print(estes_park_outdoor_demand_df.dtypes)
estes_park_outdoor_demand_df_selection = estes_park_outdoor_demand_df.iloc[46:63, 2:14]
estes_park_outdoor_demand_df_selection_update = pd.DataFrame(estes_park_outdoor_demand_df_selection.values.ravel(),columns = ['column'])
estes_park_outdoor_demand_df_selection_update['date'] = pd.date_range(start='1/1/1996', periods=len(estes_park_outdoor_demand_df_selection_update), freq='M')
estes_park_outdoor_demand_df_selection_update.index = estes_park_outdoor_demand_df_selection_update['date']
estes_park_outdoor_demand_df_selection_update['month'] = estes_park_outdoor_demand_df_selection_update.index.month
estes_park_outdoor_demand_df_selection_update['year'] = estes_park_outdoor_demand_df_selection_update.index.year
estes_park_outdoor_demand_df_selection_update['norm'] = np.nan



yearly_sum = estes_park_outdoor_demand_df_selection_update.groupby('year').sum()['column']


for i in range(0,204):
    year_check = yearly_sum.index == estes_park_outdoor_demand_df_selection_update['year'][i]
    annual_sum = yearly_sum[year_check]
    estes_park_outdoor_demand_df_selection_update['norm'][i] = estes_park_outdoor_demand_df_selection_update['column'][i]/annual_sum

monthly_mean = estes_park_outdoor_demand_df_selection_update.groupby('month').mean()['norm']
monthly_std = estes_park_outdoor_demand_df_selection_update.groupby('month').std()['norm']
monthly_std

estes_park_outdoor_demand_df_selection_update['deseas'] = estes_park_outdoor_demand_df_selection_update['norm'].copy()
for i in range(1, 13):
    mu = monthly_mean[i]
    sigma = monthly_std[i]
    estes_park_outdoor_demand_df_selection_update['deseas'].loc[estes_park_outdoor_demand_df_selection_update['month'] == i] = (estes_park_outdoor_demand_df_selection_update['deseas'].loc[estes_park_outdoor_demand_df_selection_update['month'] == i] - mu) / sigma
    
from statsmodels.graphics.tsaplots import plot_acf
# plt.figure()
# fig = plot_acf(estes_park_outdoor_demand_df_selection_update['deseas'])

estes_park_outdoor_demand_df_selection_update = estes_park_outdoor_demand_df_selection_update.fillna(0)

estes_park_outdoor_demand_df_selection_update['deseas_l1'] = np.nan
estes_park_outdoor_demand_df_selection_update['deseas_l12'] = np.nan
print(estes_park_outdoor_demand_df_selection_update.head())
print()

estes_park_outdoor_demand_df_selection_update['deseas_l1'].iloc[1:] = estes_park_outdoor_demand_df_selection_update['deseas'].values[:-1]
estes_park_outdoor_demand_df_selection_update['deseas_l12'].iloc[12:] = estes_park_outdoor_demand_df_selection_update['deseas'].values[:-12]

estes_park_outdoor_demand_df_selection_update.head

lm_log_ar = sm.ols('deseas ~ deseas_l1 + deseas_l12', data=estes_park_outdoor_demand_df_selection_update)
lm_log_ar_fit = lm_log_ar.fit()
print(lm_log_ar_fit.summary())

estes_park_outdoor_demand_df_selection_update['ar_resid'] = lm_log_ar_fit.resid
print(estes_park_outdoor_demand_df_selection_update)

# plt.figure()
# plt.plot(estes_park_outdoor_demand_df_selection_update['ar_resid'])

# plt.figure()
# plt.hist(estes_park_outdoor_demand_df_selection_update['ar_resid'])

# plt.figure()
# fig = plot_acf(estes_park_outdoor_demand_df_selection_update['ar_resid'].iloc[12:])

estes_park_outdoor_demands = pd.DataFrame()


from scipy.stats import skewnorm, gamma, norm
estes_park_outdoor_demands['synthetic2_noise'] = norm.rvs(estes_park_outdoor_demand_df_selection_update['ar_resid'].mean(), estes_park_outdoor_demand_df_selection_update['ar_resid'].std(), size=synthetic_series_length)

# plt.figure()
# plt.plot(estes_park_outdoor_demand_df_selection_update['ar_resid'])
# plt.plot(estes_park_outdoor_demands['synthetic2_noise'])

def predict_ar(lag1, lag12):
    return lm_log_ar_fit.params[0] + lm_log_ar_fit.params[1] * lag1 + lm_log_ar_fit.params[2] * lag12

max_lag = 12
nrow = estes_park_outdoor_demands.shape[0]

synth_ar = list(estes_park_outdoor_demand_df_selection_update['deseas'].iloc[-max_lag:])
synth_ar
...
for i in range(nrow):
    ...
    lag1 = synth_ar[i + max_lag - 1]
    prediction = predict_ar(lag1, lag12)
    noise = estes_park_outdoor_demands['synthetic2_noise'].iloc[i]
    synth_ar.append(prediction + noise)
    
estes_park_outdoor_demands['synthetic2_deseas'] = synth_ar[-nrow:]
estes_park_outdoor_demands

estes_park_outdoor_demands['date'] = pd.date_range(start='1/1/1950', periods=len(estes_park_outdoor_demands), freq='M')
estes_park_outdoor_demands.index = estes_park_outdoor_demands['date']
estes_park_outdoor_demands['month'] = estes_park_outdoor_demands.index.month
estes_park_outdoor_demands['year'] = estes_park_outdoor_demands.index.year


estes_park_outdoor_demands['synthetic_demand'] = estes_park_outdoor_demands['synthetic2_deseas'].copy()
for i in range(1, 13):
    mu = monthly_mean[i]
    sigma = monthly_std[i]
    estes_park_outdoor_demands['synthetic_demand'].loc[estes_park_outdoor_demands['month'] == i] = estes_park_outdoor_demands['synthetic_demand'].loc[estes_park_outdoor_demands['month'] == i] * sigma + mu

estes_park_outdoor_demands['synthetic_demand'][estes_park_outdoor_demands['synthetic_demand'] < 0] = 0 
    
estes_park_outdoor_demands['6000'] = estes_park_outdoor_demands['synthetic_demand']*6000

# plt.figure()
# plt.plot(estes_park_outdoor_demands['synthetic_demand'])


# plt.figure()
# plt.title('Estes Park Outdoor Demands')
# plt.plot(estes_park_outdoor_demands['6000'])

# plt.figure()
# plt.title('Estes Park Outdoor Historical')
# plt.plot(estes_park_outdoor_demand_df_selection_update['column'])

#GREELEY

greeley_demand_df = df.loc[df['id'] == '0400702']
greeley_demand_df['yr'] = greeley_demand_df['yr'].astype(int)
print(greeley_demand_df.dtypes)
greeley_demand_df_selection = greeley_demand_df.iloc[46:63, 2:14]
greeley_demand_df_selection_update = pd.DataFrame(greeley_demand_df_selection.values.ravel(),columns = ['column'])
greeley_demand_df_selection_update['date'] = pd.date_range(start='1/1/1996', periods=len(greeley_demand_df_selection_update), freq='M')
greeley_demand_df_selection_update.index = greeley_demand_df_selection_update['date']
greeley_demand_df_selection_update['month'] = greeley_demand_df_selection_update.index.month
greeley_demand_df_selection_update['year'] = greeley_demand_df_selection_update.index.year
greeley_demand_df_selection_update['norm'] = np.nan



yearly_sum = greeley_demand_df_selection_update.groupby('year').sum()['column']


for i in range(0,204):
    year_check = yearly_sum.index == greeley_demand_df_selection_update['year'][i]
    annual_sum = yearly_sum[year_check]
    greeley_demand_df_selection_update['norm'][i] = greeley_demand_df_selection_update['column'][i]/annual_sum

monthly_mean = greeley_demand_df_selection_update.groupby('month').mean()['norm']
monthly_std = greeley_demand_df_selection_update.groupby('month').std()['norm']
monthly_std

greeley_demand_df_selection_update['deseas'] = greeley_demand_df_selection_update['norm'].copy()
for i in range(1, 13):
    mu = monthly_mean[i]
    sigma = monthly_std[i]
    greeley_demand_df_selection_update['deseas'].loc[greeley_demand_df_selection_update['month'] == i] = (greeley_demand_df_selection_update['deseas'].loc[greeley_demand_df_selection_update['month'] == i] - mu) / sigma
    
from statsmodels.graphics.tsaplots import plot_acf
# plt.figure()
# fig = plot_acf(greeley_demand_df_selection_update['deseas'])

greeley_demand_df_selection_update = greeley_demand_df_selection_update.fillna(0)

greeley_demand_df_selection_update['deseas_l1'] = np.nan
greeley_demand_df_selection_update['deseas_l12'] = np.nan
print(greeley_demand_df_selection_update.head())
print()

greeley_demand_df_selection_update['deseas_l1'].iloc[1:] = greeley_demand_df_selection_update['deseas'].values[:-1]
greeley_demand_df_selection_update['deseas_l12'].iloc[12:] = greeley_demand_df_selection_update['deseas'].values[:-12]

greeley_demand_df_selection_update.head

lm_log_ar = sm.ols('deseas ~ deseas_l1 + deseas_l12', data=greeley_demand_df_selection_update)
lm_log_ar_fit = lm_log_ar.fit()
print(lm_log_ar_fit.summary())

greeley_demand_df_selection_update['ar_resid'] = lm_log_ar_fit.resid
print(greeley_demand_df_selection_update)

# plt.figure()
# plt.plot(greeley_demand_df_selection_update['ar_resid'])

# plt.figure()
# plt.hist(greeley_demand_df_selection_update['ar_resid'])

# plt.figure()
# fig = plot_acf(greeley_demand_df_selection_update['ar_resid'].iloc[12:])

greeley_demands = pd.DataFrame()


from scipy.stats import skewnorm, gamma, norm
greeley_demands['synthetic2_noise'] = norm.rvs(greeley_demand_df_selection_update['ar_resid'].mean(), greeley_demand_df_selection_update['ar_resid'].std(), size=synthetic_series_length)

# plt.figure()
# plt.plot(greeley_demand_df_selection_update['ar_resid'])
# plt.plot(greeley_demands['synthetic2_noise'])

def predict_ar(lag1, lag12):
    return lm_log_ar_fit.params[0] + lm_log_ar_fit.params[1] * lag1 + lm_log_ar_fit.params[2] * lag12

max_lag = 12
nrow = greeley_demands.shape[0]

synth_ar = list(greeley_demand_df_selection_update['deseas'].iloc[-max_lag:])
synth_ar
...
for i in range(nrow):
    ...
    lag1 = synth_ar[i + max_lag - 1]
    prediction = predict_ar(lag1, lag12)
    noise = greeley_demands['synthetic2_noise'].iloc[i]
    synth_ar.append(prediction + noise)
    
greeley_demands['synthetic2_deseas'] = synth_ar[-nrow:]
greeley_demands

greeley_demands['date'] = pd.date_range(start='1/1/1950', periods=len(greeley_demands), freq='M')
greeley_demands.index = greeley_demands['date']
greeley_demands['month'] = greeley_demands.index.month
greeley_demands['year'] = greeley_demands.index.year


greeley_demands['synthetic_demand'] = greeley_demands['synthetic2_deseas'].copy()
for i in range(1, 13):
    mu = monthly_mean[i]
    sigma = monthly_std[i]
    greeley_demands['synthetic_demand'].loc[greeley_demands['month'] == i] = greeley_demands['synthetic_demand'].loc[greeley_demands['month'] == i] * sigma + mu

greeley_demands['synthetic_demand'][greeley_demands['synthetic_demand'] < 0] = 0 
    
greeley_demands['6000'] = greeley_demands['synthetic_demand']*6000

# plt.figure()
# plt.plot(greeley_demands['synthetic_demand'])


# plt.figure()
# plt.title('Greeley Demands')
# plt.plot(greeley_demands['6000'])

# plt.figure()
# plt.title('Greeley Historical')
# plt.plot(greeley_demand_df_selection_update['column'])
...
synthetic_demands['0400518_I'] = estes_park_indoor_demands['synthetic_demand']
synthetic_demands['0400518_O'] = estes_park_outdoor_demands['synthetic_demand']
synthetic_demands['0400702'] = greeley_demands['synthetic_demand']
# synthetic_demands['0401001'] = bigthompowerplant_demands['synthetic_demand']
# synthetic_demands['0401002'] = hansenfeederww_demands['synthetic_demand']
...
historical_means['0400518_I'] = estes_park_indoor_demand_df_selection_update['column'].mean()*12
historical_means['0400518_O'] = estes_park_outdoor_demand_df_selection_update['column'].mean()*12
historical_means['0400702'] = greeley_demand_df_selection_update['column'].mean()*12
# historical_means['0401001'] = bigthompowerplant_demand_df_selection_update['column'].mean()*12
# historical_means['0401002'] = hansenfeederww_demand_df_selection_update['column'].mean()*12
