...
template_file = os.path.join(data_dir, "SP2016.weh")
...
statemod_well_data_df = pd.DataFrame(d)


# convert value column types to float
statemod_well_data_df[value_columns] = statemod_well_data_df[value_columns].astype(np.float64)

statemod_well_data_df

statemod_well_data_df_selection = statemod_well_data_df.loc[statemod_well_data_df['yr'] == '2010']

statemod_well_data_2010 = {}
statemod_well_data_use = {}
for i in ids.irrigation_structure_ids_list:
    statemod_well_data_2010[i] = statemod_well_data_df_selection.loc[statemod_well_data_df_selection['id'] == i]
    statemod_well_data_2010[i]['yr'] = statemod_well_data_2010[i]['yr'].astype(int)
    statemod_well_data_use[i] = statemod_well_data_2010[i].iloc[:, 2:14]
    statemod_well_data_use[i] = pd.DataFrame(statemod_well_data_use[i].values.ravel(),columns = ['demand'])
    statemod_well_data_use[i] = pd.concat([statemod_well_data_use[i]]*63, ignore_index=True)
    statemod_well_data_use[i]['date'] = pd.date_range(start='01/1/1950', periods=len(statemod_well_data_use[i]), freq='M')
    statemod_well_data_use[i].index = statemod_well_data_use[i]['date']
    statemod_well_data_use[i]['month'] = statemod_well_data_use[i].index.month
    statemod_well_data_use[i]['year'] = statemod_well_data_use[i].index.year
    statemod_well_data_use[i]['index'] = range(0,756)
    statemod_well_data_use[i].index = statemod_well_data_use[i]['index']

statemod_well_data_use_adapted_df = pd.DataFrame()

for i in ids.irrigation_structure_ids_list:
    statemod_well_data_use_adapted_df[f"{i}"] = statemod_well_data_use[i]['demand']
statemod_well_data_use_adapted_df = statemod_well_data_use_adapted_df.dropna(how='all', axis=1)
print(statemod_well_data_use_adapted_df)
...
statemod_well_data = read_text_file('SP2016.weh')

start_year = 1950

def writenewWEH(statemod_well_data, statemod_well_data_use_adapted_df, start_year, scenario_name):    
  new_data = []
  use_value = 0
  start_loop = 0
  col_start = [0, 5, 17, 25, 33, 41, 49, 57, 65, 73, 81, 89, 97, 105, 113]
  for n in ids.irrigation_structure_ids_list: 
      for i in range(0, len(statemod_well_data)):
        if use_value == 1:
          start_loop = 1
        if statemod_well_data[i][0] != '#':
          use_value = 1
        if start_loop == 1:
          use_line = True
          row_data = []
          ...
          for col_loc in range(0, len(col_start)):
            if col_loc == len(col_start) - 1:
              value_use.append(statemod_well_data[i][col_start[col_loc]:].strip())
            else:          
              value_use.append(statemod_well_data[i][col_start[col_loc]:(col_start[col_loc + 1])].strip())
          try:
          ...
          if structure_name in ids.irrigation_structure_ids_list:
            ...
            for i in range(0,12):
                new_demands[i] = statemod_well_data_use_adapted_df.loc[index_val + i, structure_name]
      ...
      f = open('SP2016_' + scenario_name + '.weh','w')
      # write firstLine # of rows as in initial file
      i = 0
      while statemod_well_data[i][0] == '#':
        f.write(statemod_well_data[i])
        i += 1
      f.write(statemod_well_data[i])
      i+=1
...
writenewWEH(statemod_well_data, statemod_well_data_use_adapted_df, 1950, 'A')
