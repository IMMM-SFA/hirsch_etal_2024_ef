...
fp8 = 'South Platte Watershed Basin.shp'
sprb_boundary = gpd.read_file(fp8)
sprb_boundary = sprb_boundary.to_crs("epsg:3857")
sprb_boundary[['geometry']].plot(facecolor="none", edgecolor="black")

fp9 = 'Watersheds.shp'
ucrb_sub_boundary = gpd.read_file(fp9)
ucrb_sub_boundary = ucrb_sub_boundary.to_crs("epsg:3857")
ucrb_sub_boundary[['geometry']].plot(facecolor="none", edgecolor="black")
...
# northern_water_boundary[['geometry']].plot(facecolor="yellow", edgecolor="blue", alpha = 0.3, ax=ax, label = 'Northern Water Service Area')
# colorado_state_boundary[['geometry']].plot(facecolor="none", edgecolor="black",linewidth = 5.5,  ax=ax)
# #ucrb_sub_boundary[['geometry']].plot(facecolor="none", edgecolor="black", linewidth = 2, ax=ax, label = 'Upper Colorado River Basin')
# sprb_boundary[['geometry']].plot(facecolor="none", edgecolor="black", linewidth = 2, ax=ax, label = 'South Platte River Basin')
...
# # Add arrows using matplotlib's annotate function with an arrow marker at the centroid of each geometry
...
# handles.append(northern_water_boundary[['geometry']].plot(facecolor="yellow", edgecolor="blue", alpha = 0.3, ax=ax).collections[0])
# labels.append('Northern Water Service Area')
# #ax.legend(handles=handles, labels=labels)
# import matplotlib.patches as mpatches

# # create a new patch object
# new_patch = mpatches.Patch(color='yellow', label='Northern Water Service Area')

# # get the current handles and labels from the legend
# handles, labels = ax.get_legend_handles_labels()

# # add the new patch object to the handles and labels
# handles.append(new_patch)
# labels.append('Northern Water Service Area')

# # update the legend with the new handles and labels
# ax.legend(handles=handles, labels=labels, loc = 'lower left', fontsize = 13, bbox_to_anchor=(0.008,0.97))
...
total_acreages = map_df_aggregated_crops.groupby(['CROP_TYPE'], as_index=False)["ACRES"].sum()
...
yearly_water_values_forattach_dry_SI = structures_by_yearly_water_value.transpose()
yearly_water_values_forattach_dry_SI = yearly_water_values_forattach_dry_SI.multiply(1233.4818375475).divide(1000000)
#yearly_water_values_forattach_dry_SI = yearly_water_values_forattach_dry_SI.drop('StateMod_Structure', axis=1)

max_value_dry = yearly_water_values_forattach_dry.to_numpy().max()
max_value_dry_SI = yearly_water_values_forattach_dry_SI.to_numpy().max()
columnnames = {}
count = 1949
...
count = 1949
for i in yearly_water_values_forattach_dry_SI.columns:
    count +=1
    columnnames[i] = f"value_{count}_dry"
yearly_water_values_forattach_dry_SI.rename(columns = columnnames, inplace = True)
yearly_water_values_forattach_dry_SI.columns

yearly_water_values_forattach_dry['StateMod_Structure'] = yearly_water_values_forattach_dry.index
#map_df_update = map_df_update.drop('StateMod_Structure', axis=1)

yearly_water_values_forattach_dry_SI['StateMod_Structure'] = yearly_water_values_forattach_dry_SI.index
map_df_update = map_df_update.drop('StateMod_Structure', axis=1)


map_df_update2 = pd.merge(map_df_update, yearly_water_values_forattach_dry, on="StateMod_Structure")
map_df_update2_SI = pd.merge(map_df_update, yearly_water_values_forattach_dry_SI, on="StateMod_Structure")
...
map_df_update2_SI = gpd.GeoDataFrame(map_df_update2_SI)
map_df_update2_SI = map_df_update2_SI.to_crs("epsg:3857")
map_df_update2_SI.plot()
...
# Define a list of hex code colors
hex_colors = ['#335C67', '#66827A', '#99A88C', '#BDA465', '#E09F3E', '#9A5526', '#540B0E']

# Create a custom colormap from the list of colors
cmap_test = mcolors.ListedColormap(hex_colors)

#max_value_dry_SI = (max_value_dry*1233.4818375475)/1000000
...
for i in range(1955,1956):
    ...
    norm1 = colors.Normalize(vmin=0, vmax=max_value_dry_SI)
    cbar = plt.cm.ScalarMappable(norm=norm1, cmap=cmap_test)

    fig, ax = plt.subplots(1, figsize =(24, 8))
    ax.set_ylim([4820000, 5050000])
    ax.set_xlim([-11780000, -11375000])
    map_df_update2_SI.plot(column= f"value_{i}_dry", categorical=False, cmap=cmap_test, linewidth=.1, edgecolor='0.2',
                legend=False, ax=ax)
    ...
    ax_cbar.set_label(label='Marginal Value of Water ($/MCM)',size = 15)
    #plt.legend(loc="upper right")
    ax.axis('off')
    ax.grid (False)
    #ax.set_title('South Platte Two-Way Option Market in Driest Hydrologic Year',fontsize=20, weight='bold')
    src_basemap = cx.providers.CartoDB.Voyager
    cx.add_basemap( ax, source=src_basemap, crs = map_df_update2_SI.crs )
    plt.tight_layout()
    plt.savefig('MVH2O_map_2.png', format = 'png', dpi=600)
...
for i in range(1955,1956):
    ...
    for crop in (range(len(structure_uses_by_year_dry[i]))):  
        ...
        structure_uses_by_year_dry[i]['CUM_USAGE_SI'] = structure_uses_by_year_dry[i]['CUM_USAGE'].multiply(1233.4818375475).divide(1000000)
        structure_uses_by_year_dry[i]['MARGINAL_VALUE_OF_WATER_SI'] = structure_uses_by_year_dry[i]['MARGINAL_VALUE_OF_WATER'].divide(1233.4818375475)
...
for i in range(1955,1956):
    ...
    ax.scatter(data=structure_uses_by_year_dry[i], x='CUM_USAGE_SI', y='MARGINAL_VALUE_OF_WATER_SI', c='black')
    ax.set(xlabel='Available Water Supply (MCM)', ylabel='Marginal Value of Water ($/MCM)')
    ...
    ax.set_ylim ([0, 0.25])
    #plt.savefig('figure_5_supply_fn_1955.png', format = 'png', dpi=600)

# for i in range(1955,1956):
#     fig, ax = plt.subplots(figsize=(11, 4))
#     ax.scatter(data=structure_uses_by_year_dry[i], x='CUM_USAGE', y='MARGINAL_VALUE_OF_WATER', cmap=cmap_test)
...
# for i in range(1955,1956):
#     fig, ax = plt.subplots(figsize=(11, 4))
#     ax.scatter(data=structure_uses_by_year_dry[i], x='CUM_USAGE', y='MARGINAL_VALUE_OF_WATER', c='black')
#     ax.set(xlabel='Available Water Supply (AF)', ylabel='Marginal Value of Water ($)')
#     #ax.vlines(structure_uses_by_year_dry[1954]['CUM_USAGE'], 0, structure_uses_by_year_dry[1954]['MARGINAL_VALUE_OF_WATER'], linestyle="solid", color = 'NUMERIC_CROP_TYPE')
#     ax.set_ylim ([0, 300])
#     plt.savefig('supply_fn_1955.png', format = 'png', dpi=600)
...
#     # create the colorbar
#     norm2 = colors.Normalize(vmin=0, vmax=max_value_wet)
...
for y in range(1950,2013):
    ...
    if y in [1950, 1952, 1958, 1962, 1969, 1970, 1971, 1972, 1973, 1974, 1980, 1984, 1985, 1986, 1988, 1989, 1996, 1997, 1998, 2000, 2006, 2011]:
        ...
        muni_surplus_available = surplus.Lease_Amounts_by_CBI[y]
    else:
