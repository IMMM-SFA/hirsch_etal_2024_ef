...
os.chdir('C:/Users/zacha/Documents/UNC/SP2016_StateMod/SP_historicalrights_test')
import new_irrigation_test_historicalrights as old
import seaborn as sns
from scipy.stats import norm
from scipy.integrate import quad

scenario_no = 3
scenario_type = 'historical water rights'
...
## import CBI ###
## CREATE THRESHOLDS BASED ON DIFFERENT CBI VALUES ##
...
for y in range(1950,2013):
    ...
    if y in CBI_S2.index:
        ...
        max_mv = list(old.TWO_Selection[y].loc[old.TWO_Selection[y]['TWO_USAGE'] > 0]['MARGINAL_VALUE_OF_WATER'])[-1]
        max_mv_dict[y] = max_mv
        print(max_mv)
        ag_max_rev_price = 1000
        old.TWO_Selection[y]['TWO_BASELINE_SCENARIO_MV'] = old.TWO_Selection[y]['TWO_USAGE']*ag_max_rev_price
        for n in exercise_fees:
            old.TWO_Selection[y][f"exercise_fee_{n}"] = old.TWO_Selection[y]['TWO_USAGE']*n
            old.TWO_Selection[y][f"TWO_BASELINE_SCENARIO_E{n}"] = old.TWO_Selection[y]['TWO_BASELINE_SCENARIO_MV']-old.TWO_Selection[y][f"exercise_fee_{n}"] 
    else:
        old.TWO_Selection[y]['TWO_BASELINE_SCENARIO_MV'] = 0
    print(old.TWO_Selection[y])
    added_mv_dry[y]= old.TWO_Selection[y]['TWO_BASELINE_SCENARIO_MV'].sum()

dry_year_options = {}
individual_dry_year_contracts = pd.DataFrame()
for i in (old.TWO_Selection[y].index.unique()):
    contracts = pd.DataFrame()
    for y in range(1950,2013):
        contract = old.TWO_Selection[y].loc[old.TWO_Selection[y].index== i] #could be uses_of_water_dry in [] if things break
...
for n in exercise_fees:
    ...
    for i in (old.uses_of_water_dry[y].index.unique()): 
...
for e in exercise_fees:
    ...
    for name in (old.uses_of_water_dry[y].index.unique()):
...
exercise_fees = [5,10,15,20,25]
added_mv_wet = pd.Series(index = range(1950,2013))
min_mv_dict = {}
for y in range(1950,2013):
    old.TWO_Selection_Wet[y]['year'] = y
    old.TWO_Selection_Wet[y]['TWO_MV'] = 0
    #max_mv = list(old.TWO_Selection[y].loc[old.TWO_Selection[y]['TWO_USAGE'] > 0]['MARGINAL_VALUE_OF_WATER'])[-1]
    if y in CBI_Surplus.index:
        #max_mv = list(old.TWO_Selection[y].loc[old.TWO_Selection[y]['TWO_USAGE'] > 0]['MARGINAL_VALUE_OF_WATER'])[-1]
        #list(old.TWO_Selection[y].loc[old.TWO_Selection[y]['TWO_USAGE'] > 0]['MARGINAL_VALUE_OF_WATER'])[-1]
        min_mv = list(old.TWO_Selection_Wet[y].loc[old.TWO_Selection_Wet[y]['TWO_USAGE'] > 0]['MARGINAL_VALUE_OF_WATER'])[-1]
        min_mv_dict[y] = min_mv
        old.TWO_Selection_Wet[y]['TWO_MV'] = old.TWO_Selection_Wet[y]['TWO_USAGE'] * old.TWO_Selection_Wet[y]['MARGINAL_VALUE_OF_WATER']
        old.TWO_Selection_Wet[y]['TWO_BASELINE_SCENARIO_MVH2O'] = old.TWO_Selection_Wet[y]['TWO_USAGE']*min_mv
        old.TWO_Selection_Wet[y]['TWO_BASELINE_SCENARIO_MV30'] = old.TWO_Selection_Wet[y]['TWO_USAGE']*30
        for n in exercise_fees:
            old.TWO_Selection_Wet[y][f"exercise_fee_{n}"] = old.TWO_Selection_Wet[y]['TWO_USAGE']*n
            old.TWO_Selection_Wet[y][f"TWO_BASELINE_SCENARIO_MVH2O_E{n}"] = old.TWO_Selection_Wet[y]['TWO_BASELINE_SCENARIO_MVH2O']-old.TWO_Selection_Wet[y][f"exercise_fee_{n}"]
            old.TWO_Selection_Wet[y][f"TWO_BASELINE_SCENARIO_MV30_E{n}"] = old.TWO_Selection_Wet[y]['TWO_BASELINE_SCENARIO_MV30']-old.TWO_Selection_Wet[y][f"exercise_fee_{n}"]
    else:
        old.TWO_Selection_Wet[y]['TWO_BASELINE_SCENARIO_MVH2O'] = 0
        old.TWO_Selection_Wet[y]['TWO_BASELINE_SCENARIO_MV30'] = 0
    print(old.TWO_Selection_Wet[y])
    added_mv_wet[y]= old.TWO_Selection_Wet[y]['TWO_MV'].sum()

wet_year_options = {}
individual_wet_year_contracts = pd.DataFrame()
for i in (old.TWO_Selection_Wet[y].index.unique()):
    contracts_wet = pd.DataFrame()
    for y in range(1950,2013):
        contracts_wet = old.TWO_Selection_Wet[y].loc[old.TWO_Selection_Wet[y].index== i] #could be uses_of_water_dry in [] if things break
...
for n in exercise_fees:
    ...
    for i in (old.uses_of_water_wet[y].index.unique()): 
...
for e in exercise_fees:
    ...
    for name in (old.uses_of_water_wet[y].index.unique()):
...
# fig.suptitle('Two-Way Option (Baseline)', weight = 'bold' )
...
dry_year_analysis_exercise_fee = 700
wet_year_analysis_exercise_fee = 25
...
# for label in ax2.get_yticklabels()[0:13]:
#     label.set_color("black")
#     label.set_fontsize(20)
#     label.set_fontweight('bold')
# # ticks = ax2.get_yticks()
# # ax2.set_yticklabels([int(abs(tick)) for tick in ticks], fontsize=15, weight = 'bold')
...
#     label.set_color("blue")
# # set labels to absolute values and with integer representation
# ax3.set_yticklabels([int(abs(tick)) for tick in ticks], fontsize=20, weight = 'bold')

# ax3.axhline(y=0, color='black')
# #plt.ylabel('TWO Cost ($)')
# ax3.text(-.1, .75, 'Ag', ha='center', va='center', transform=ax3.transAxes, rotation='vertical', color = 'blue', weight = 'bold', fontsize = 20)
# ax3.text(-.1, .25, 'Urban', ha='center', va='center', transform=ax3.transAxes, rotation='vertical', color = 'red', weight = 'bold', fontsize = 20)
...
# Set the color of your plot using a hex code
color1 = '#335C67'
color2 = '#540B0E'
color3 = '#99A88C'
color4 = '#E09F3E'
...
# set labels to absolute values and with integer representation
ax1.set_yticklabels([int(abs(tick)) for tick in ticks], fontsize=20)

ax1.axhline(y=0, color='black')
twin1 = plt.twinx()
twin1.plot(x, CBI_Yearly, color = color5) 
twin1.set_ylabel('CBI', color = color5, fontsize=20) 
twin1.tick_params(labelcolor = color5, labelsize=20)
ticks =  twin1.get_yticks()
twin1.set_yticklabels([int(abs(tick)) for tick in ticks], fontsize=20)
ax1.text(-.1, 0.75, 'To Ag', ha='center', va='center', transform=ax1.transAxes, rotation='vertical', color = color1, fontsize=20)
ax1.text(-.1, 0.25, 'To Urban', ha='center', va='center', transform=ax1.transAxes, rotation='vertical', color = color2, fontsize=20)
ax1.text(-.15, 0.50, 'Water Re-allocated (kAF)', ha='center', va='center', transform=ax1.transAxes, rotation='vertical', color = 'black', fontsize=20)
...
plt.ylim([-2,21])
ax2.text(-.15, .5, 'Ag Gains/Losses ($M)', ha='center', va='center', transform=ax2.transAxes, rotation='vertical', color = 'black', fontsize=20)
...
# set labels to absolute values and with integer representation
ax3.set_yticklabels([int(abs(tick)) for tick in ticks], fontsize=20)

ax3.axhline(y=0, color='black')
#plt.ylabel('TWO Cost ($)')
ax3.text(-.1, .75, 'Ag', ha='center', va='center', transform=ax3.transAxes, rotation='vertical', color = color6, fontsize = 20)
ax3.text(-.1, .25, 'Urban', ha='center', va='center', transform=ax3.transAxes, rotation='vertical', color = color7, fontsize = 20)
ax3.text(-.15, 0.45, 'Two-Way Option Cost ($M)', ha='center', va='center', transform=ax3.transAxes, rotation='vertical', color = 'black', fontsize=20)
ax3.text(0.985, 0.985, 'B', fontsize=30, fontname='Myriad Pro', transform=ax3.transAxes, ha='right', va='top')
plt.subplots_adjust(hspace=0.2)

ticks =  ax1.get_xticks()
ax1.set_xticklabels([int(abs(tick)) for tick in ticks], fontsize=15)
ax2.set_xticklabels([int(abs(tick)) for tick in ticks], fontsize=15)
ax3.set_xticklabels([int(abs(tick)) for tick in ticks], fontsize=15)
#plt.show()
plt.savefig('TWO_historical_S3_hi_res.png', format = 'png', dpi=600, bbox_inches='tight')

sum_ag_nb = two_nb_to_ag + added_mv_wet
...
# fig, (ax1, ax2) = plt.subplots(nrows=2, ncols=1, dpi=500, sharex=True)
# fig.suptitle('Two-Way Option (Scenario 2)', weight = 'bold' )
