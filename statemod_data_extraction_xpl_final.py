...
# SBATCH -t 03:00:00
# SBATCH --exclusive
# SBATCH --job-name xpl_to_parquet
...
class StateModXPLDataExtractor:
    """Class to handle extracting structure, sample, and realization data from StateMod xpl files."""

    def __init__(
            self,
            structure_ids_file_path: str,
            output_path: str,
            glob_to_xpl: str = None,
            xpl_files: Iterable[str] = None,
        ...
        # glob path to xpl files
        # absolute path is best; relative path must be relative to where you run from
        self.glob_to_xpl = glob_to_xpl
        self.files = xpl_files
        if self.glob_to_xpl is None and self.files is None:
            raise IOError("Must supply at least one xpl file.")
        ...
        self.riverid_column = 1
        self.riverid_column_name = 'river_id'
        self.riverid_column_type = object
        ...
        self.totalsupply_column = 4
        self.totalsupply_column_name = 'totalsupply'
        self.totalsupply_column_type = np.uint32
        
        self.use1_column = 5
        self.use1_column_name = 'use1'
        self.use1_column_type = np.uint32
        
        self.use2_column = 6
        self.use2_column_name = 'use2'
        self.use2_column_type = np.uint32
        
        self.use3_column = 7
        self.use3_column_name = 'use3'
        self.use3_column_type = np.uint32
        
        self.use4_column = 8
        self.use4_column_name = 'use4'
        self.use4_column_type = np.uint32
        
        self.use5_column = 9
        self.use5_column_name = 'use5'
        self.use5_column_type = np.uint32
        
        self.use6_column = 10
        self.use6_column_name = 'use6'
        self.use6_column_type = np.uint32
        
        self.use7_column = 11
        self.use7_column_name = 'use7'
        self.use7_column_type = np.uint32
        
        self.use8_column = 12
        self.use8_column_name = 'use8'
        self.use8_column_type = np.uint32
        
        self.use9_column = 13
        self.use9_column_name = 'use9'
        self.use9_column_type = np.uint32
        
        self.use10_column = 14
        self.use10_column_name = 'use10'
        self.use10_column_type = np.uint32
        
        self.use11_column = 15
        self.use11_column_name = 'use11'
        self.use11_column_type = np.uint32
        
        self.use12_column = 16
        self.use12_column_name = 'use12'
        self.use12_column_type = np.uint32
        
        self.use13_column = 17
        self.use13_column_name = 'use13'
        self.use13_column_type = np.uint32
        
        self.use14_column = 18
        self.use14_column_name = 'use14'
        self.use14_column_type = np.uint32
        
        self.use15_column = 19
        self.use15_column_name = 'use15'
        self.use15_column_type = np.uint32
        
        self.use16_column = 20
        self.use16_column_name = 'use16'
        self.use16_column_type = np.uint32
        
        self.use17_column = 21
        self.use17_column_name = 'use17'
        self.use17_column_type = np.uint32
        
        self.use18_column = 22
        self.use18_column_name = 'use18'
        self.use18_column_type = np.uint32
        
        self.use19_column = 23
        self.use19_column_name = 'use19'
        self.use19_column_type = np.uint32
        
        self.use20_column = 24
        self.use20_column_name = 'use20'
        self.use20_column_type = np.uint32
        
        self.total_column = 25
        self.total_column_name = 'total'
        self.total_column_type = np.uint32
        ...
        self.realization_number_regex = re.compile(r'_(\d+)(?:\.xpl)?$')
        ...
        self.expected_column_sizes = np.asarray([
            13, 13, 6, 5, 8, 8, 8, 8, 8, 8, 8, 8, 8, 8, 8, 8, 8, 8, 8, 8, 8, 8, 8, 8, 8, 6
        ])
            
        # the line separator counts as an extra character, hence the +1
        self.expected_line_size = self.expected_column_sizes.sum() + 1

    def parse_xpl_file(self, file_path: str) -> bool:
        """Parses a StateMod xpl file into a parquet file.

        Args:
            file_path (str): a file path to xpl file
        ...
        df = pd.read_csv(
            ...
            names=[
                self.id_column_name,
                self.riverid_column_name,
                self.year_column_name,
                self.month_column_name,
                self.totalsupply_column_name,
                self.use1_column_name,
                self.use2_column_name,
                self.use3_column_name,
                self.use4_column_name,
                self.use5_column_name,
                self.use6_column_name,
                self.use7_column_name,
                self.use8_column_name,
                self.use9_column_name,
                self.use10_column_name,
                self.use11_column_name,
                self.use12_column_name,
                self.use13_column_name,
                self.use14_column_name,
                self.use15_column_name,
                self.use16_column_name,
                self.use17_column_name,
                self.use18_column_name,
                self.use19_column_name,
                self.use20_column_name,
                self.total_column_name
            ],
            dtype={
                self.id_column_name: self.id_column_type,
                self.riverid_column_name : self.riverid_column_type,
                ...
                self.use1_column_name: self.use1_column_type,
                self.use2_column_name: self.use2_column_type,
                self.use3_column_name: self.use3_column_type,
                self.use4_column_name: self.use4_column_type,
                self.use5_column_name: self.use5_column_type,
                self.use6_column_name: self.use6_column_type,
                self.use7_column_name: self.use7_column_type,
                self.use8_column_name: self.use8_column_type,
                self.use9_column_name: self.use9_column_type,
                self.use10_column_name: self.use10_column_type,
                self.use11_column_name: self.use11_column_type,
                self.use12_column_name: self.use12_column_type,
                self.use13_column_name: self.use13_column_type,
                self.use14_column_name: self.use14_column_type,
                self.use15_column_name: self.use15_column_type,
                self.use16_column_name: self.use16_column_type,
                self.use17_column_name: self.use17_column_type,
                self.use18_column_name: self.use18_column_type,
                self.use19_column_name: self.use19_column_type,
                self.use20_column_name: self.use20_column_type,
                self.total_column_name: self.total_column_type
            }
        )
    ...
    def extract(self):
        ...
        files = self.files if self.files is not None and len(self.files) > 0 else glob(self.glob_to_xpl)

        # make sure we found some xdds
        if not files or len(files) == 0:
            raise IOError(f"Unable to find any files with '{self.glob_to_xpl}'")
        ...
        with context(**(dict() if self.use_mpi else dict(n_jobs=-1, temp_folder=self.temporary_path))) as executor:
            ...
            logging.info('Creating temporary parquet files per xpl.')
            if self.use_mpi:
                successful_xpl = executor.map(self.parse_xpl_file, files, unordered=True)
            else:
                successful_xpl = executor(delayed(self.parse_xpl_file)(file) for file in files)
            # check how many failed
            failed_xpl = [files[i] for i, status in enumerate(successful_xpl) if status is False]
            if len(failed_xpl) > 0:
        ...
        logging.info(
            f'Processed {len(files) - len(failed_xpl)} xpl files ' +
            f'into {len(self.ids_of_interest) - len(failed_parquet)} parquet files ' +
            f'in {self.pretty_timer(timer() - t)}.'
        )
        if len(failed_xpl) > 0 or len(failed_parquet) > 0:
            logging.error(
                f"Failed to process to {len(failed_xpl)} xpl files and {len(failed_parquet)} structure_id files."
            )
...
if __name__ == '__main__':
    ...
    if not use_mpi or mpi.COMM_WORLD.Get_rank() == 0:
        ...
        parser = argparse.ArgumentParser(
            description=
            "Extract data from XPL files for a given set of structure IDs, producing a parquet file for each ID."
        )
        parser.add_argument(
            '-f',
            '--force',
        ...
        parser.add_argument(
            ...
            metavar='file',
            nargs='+',
            help="glob to the XPL files to parse (i.e. './*.xpl')"
        )

        args = parser.parse_args()

        extractor = StateModXPLDataExtractor(
            allow_overwrite=args.force,
            xpl_files=args.files,
